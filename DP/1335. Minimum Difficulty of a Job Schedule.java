class Solution {
    ...
    private int dfs(int[][]dp , int[]jobDifficulty  , int days , int idx){
        ...
        for(int i=idx ; i<jobDifficulty.length-days+1 ;i++) {   //We are trying to put days number point of breakages so we need t iterate for i<jobDifficulty.length-days+1 else breakages will be less than days
